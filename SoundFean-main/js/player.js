...
export class Player {
    ...
    constructor(audioElement, api, quality = 'LOSSLESS') {
        ...
        this.ytPlayer = null; // YouTube IFrame controller (only when playback-source=youtube)
        this._ytProgressTimer = null;
    ...
    setVolume(value) {
        ...
        if (this.currentStreamProvider === 'youtube' && this.ytPlayer) {
            this.ytPlayer.setVolume(this.userVolume);
        }
    ...
    /**
     * Play via official YouTube IFrame API while keeping Tidal title/artist/album/cover on the track.
     */
    async _playYouTubeTrack(track, videoId, startTime = 0, sequence = null) {
        ...
        try {
            ...
            console.warn('[yt] cleanup prior stream:', e);
        }

        if (!this.ytPlayer) {
            this.ytPlayer = new YouTubePlaybackController({
        ...
        // --- Update Now Playing bar with TIDAL metadata (same as normal path) ---
        const trackTitle = getTrackTitle(track);
        const trackArtistsHTML = getTrackArtistsHTML(track);
        const yearDisplay = getTrackYearDisplay(track);
    ...
    _updateYouTubeProgressUI(currentTime, duration) {
        if (!duration || duration <= 0 || !Number.isFinite(currentTime)) return;
        const pct = Math.max(0, Math.min(100, (currentTime / duration) * 100));
        document.querySelectorAll('#progress-fill, #fs-progress-fill').forEach((el) => {
            el.style.width = `${pct}%`;
        });
        const timeText = formatTime(currentTime);
        document.querySelectorAll('#current-time, #fs-current-time').forEach((el) => {
            el.textContent = timeText;
        });
        const durText = formatTime(duration);
        document.querySelectorAll('#total-duration, #fs-total-duration, #duration').forEach((el) => {
            if (el) el.textContent = durText;
        });
    }
    ...
    _stopYouTubeProgressTimer() {
        if (this._ytProgressTimer) {
            clearInterval(this._ytProgressTimer);
            this._ytProgressTimer = null;
        }
    }
    ...
    async handlePlayPause() {
        // YouTube path: control IFrame player; UI metadata remains Tidal
        if (this.currentStreamProvider === 'youtube' && this.ytPlayer) {
